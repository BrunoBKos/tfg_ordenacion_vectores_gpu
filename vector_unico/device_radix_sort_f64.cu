...
int main(int argc, char** argv) {
  ...
  sorted_vector = (double*) malloc(N*sizeof(double));
  ...
  for(int i = 0; i < N; i++) vect[i]=((double)rand()/(double) RAND_MAX);
  ...
  //device copy
  cudaMemcpyAsync(d_vector, vect, N*sizeof(double), cudaMemcpyHostToDevice, stream);
  // Run sorting operation
  cub::DeviceRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_vector, d_sorted_vector, N); 
  // recovery of the parallel results
  cudaMemcpyAsync(sorted_vector, d_sorted_vector, N*sizeof(double), cudaMemcpyDeviceToHost, stream);
