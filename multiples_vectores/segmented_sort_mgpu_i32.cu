...
int main(int argc, char** argv) {
  ...
  vect = (int*) malloc(M*N*sizeof(int));
  sorted_vector = (int*) malloc(M*N*sizeof(int));

  // memory reserve in GPU
  cudaMalloc(&d_vector, M*N*sizeof(int));
  ...
  for(int i = 0; i < M*N; i++) vect[i] = (int) (rand()%1024);
  ...
  //device copy
  cudaMemcpy(d_vector, vect, M*N*sizeof(int), cudaMemcpyHostToDevice);
  // Run sorting operation
  mgpu::segmented_sort(d_vector, M*N, d_offsets, M, less_int(), context); 
  // recovery of the parallel results
  cudaMemcpy(sorted_vector, d_vector, M*N*sizeof(int), cudaMemcpyDeviceToHost);
