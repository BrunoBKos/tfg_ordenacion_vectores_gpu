...
int main(int argc, char** argv) {
  ...
  cudaMalloc(&d_sorted_vector, N*sizeof(double));
  ...
  cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_vector, d_sorted_vector, 
                                            M*N, M, d_offsets, d_offsets+1, 0, sizeof(double)*8);
  ...
  //device copy
  cudaMemcpyAsync(d_vector, vect, M*N*sizeof(double), cudaMemcpyHostToDevice, stream);
  // Run sorting operation
  cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_vector, d_sorted_vector, 
                                            M*N, M, d_offsets, d_offsets+1, 0, sizeof(double)*8); 
  // recovery of the parallel results
  cudaMemcpyAsync(sorted_vector, d_sorted_vector, M*N*sizeof(double), cudaMemcpyDeviceToHost, stream);
