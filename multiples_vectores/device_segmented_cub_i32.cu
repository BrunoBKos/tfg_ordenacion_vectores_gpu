...
int main(int argc, char** argv) {
  ...
  cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_vector, d_sorted_vector, 
                                            M*N, M, d_offsets, d_offsets+1, 0, sizeof(int)*8);
  ...
  //device copy
  cudaMemcpyAsync(d_vector, vect, M*N*sizeof(int), cudaMemcpyHostToDevice, stream);
  // Run sorting operation
  cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_vector, d_sorted_vector, 
                                            M*N, M, d_offsets, d_offsets+1, 0, sizeof(int)*8); 
  // recovery of the parallel results
  cudaMemcpyAsync(sorted_vector, d_sorted_vector, M*N*sizeof(int), cudaMemcpyDeviceToHost, stream);
