...
void quicksort_parallel_tasks(short arr[], int low, int high); //implementacion paralela OpenMP con "task
...
int main(int argc, char *argv[]) {
    ...
    vector = (short*) malloc(N*sizeof(short));
    output_vector = (short*) malloc(N*sizeof(short));
    cudaMalloc((void**) &device_vector, N * sizeof(short));

    initialize_vector_i16(vector, N);
    ...
    printf("Thrust CPU...\n");
    start = omp_get_wtime();
    memcpy(output_vector, vector, N*sizeof(short));
    ...
    printf("Thrust GPU...\n");
    start = omp_get_wtime();
    cudaMemcpy(device_vector, vector, N*sizeof(short), cudaMemcpyHostToDevice);
    thrust::sort(thrust::device, device_vector, device_vector + N);
    cudaMemcpy(output_vector, device_vector, N*sizeof(short), cudaMemcpyDeviceToHost);
    end = omp_get_wtime();
    printf("Tiempo: %f segundos\n", end - start);
    printf("-----------------------------------\n");
...
/* Mediana de tres */
int median_of_three(short arr[], int low, int high) {
...
int partition(short arr[], int low, int high) { 
    ...
    /* -----------------------------------------------------*/
    short pivot = arr[high];  // Ultimo elemento como pivote
    int i = low - 1;  
...
void quicksort_parallel_tasks(short arr[], int low, int high) {
    ...
    if (low < high) {
        if (high - low < threshold || threads == 1) {
            ...
            qsort(arr + low, high - low + 1, sizeof(short), compare);
        } 
        else {
