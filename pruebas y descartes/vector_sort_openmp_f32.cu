...
void quicksort_parallel_tasks(float arr[], int low, int high); //implementacion paralela OpenMP con "task
...
int main(int argc, char *argv[]) {
    ...
    output_vector = (float*) malloc(N*sizeof(float));
    cudaMalloc((void**) &device_vector, N * sizeof(float));

    initialize_vector_f32(vector, N);
    ...
    memcpy(output_vector, vector, N*sizeof(float));
    ...
    printf("Thrust GPU...\n");
    gettimeofday(&tv_start, NULL);
    cudaMemcpy(device_vector, vector, N*sizeof(float), cudaMemcpyHostToDevice);
    thrust::sort(thrust::device, device_vector, device_vector + N);
    cudaMemcpy(output_vector, device_vector, N*sizeof(float), cudaMemcpyDeviceToHost);
    gettimeofday(&tv_end, NULL);
    run_time=(tv_end.tv_sec - tv_start.tv_sec) * 1000000 + (tv_end.tv_usec - tv_start.tv_usec); //useconds
    run_time /= 1000000; // seconds
    printf("Tiempo: %lg segundos\n", run_time);
...
/* Mediana de tres */
int median_of_three(float arr[], int low, int high) {
...
int partition(float arr[], int low, int high) { 
    ...
    /* -----------------------------------------------------*/
    float pivot = arr[high];  // Ultimo elemento como pivote
    int i = low - 1;  
...
void quicksort_parallel_tasks(float arr[], int low, int high) {
    ...
    if (low < high) {
        if (high - low < threshold || threads == 1) {
            ...
            qsort(arr + low, high - low + 1, sizeof(float), compare);
        } 
        else {
