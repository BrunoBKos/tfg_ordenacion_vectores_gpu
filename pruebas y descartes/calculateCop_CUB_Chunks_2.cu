...
int main(int argc, char *argv[]) {
    ...
    #pragma omp parallel reduction(+:total_not_valid, notCalculated) num_threads(2) 
    {
    int threadId_Global = omp_get_thread_num();
    ...
    cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_input, d_output, 
                                            TIME*CHK_SZ, CHK_SZ, d_offsets, d_offsets+1, 0, sizeof(float)*8, stream);
    ...
    for(int j = 0; j < LAT; j++) {
      ...
      for(int t = 0; t < N_CHKS; t++) {
        ...
        total_not_valid = 0;
        notCalculated = 0;
        #pragma omp parallel reduction(+:total_not_valid, notCalculated) num_threads(NUM_THREADS/2)
        {
          int threadId_local = omp_get_thread_num();
          ...
          for (int k = 0; k < CHK_SZ; k++) { // lectura de los datos + calculo del drought_code
            ...
            int ret1 = nc_get_vara_short(tgFiles[(threadId_Global*(NUM_THREADS/2)) + threadId_local].ncId, tgFiles[(threadId_Global*(NUM_THREADS/2)) + threadId_local].varId, read_start, read_count, part_tg_val+(k*TIME));
            int ret2 = nc_get_vara_float(precFiles[(threadId_Global*(NUM_THREADS/2)) + threadId_local].ncId, precFiles[(threadId_Global*(NUM_THREADS/2)) + threadId_local].varId, read_start, read_count, part_prec_val+(k*TIME));
          ...
          #pragma omp single nowait
          {
            cudaMemcpyAsync(d_input, drought_code, CHK_SZ*TIME*sizeof(float), cudaMemcpyHostToDevice, stream);
            cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_input, d_output, 
                                                  TIME*CHK_SZ, CHK_SZ, d_offsets, d_offsets+1, 0, sizeof(float)*8, stream);
          }
          ...
          #pragma omp single
          {
            cudaMemcpyAsync(orderedPrec, d_output, CHK_SZ*TIME*sizeof(float), cudaMemcpyDeviceToHost, stream);
          }
          #pragma omp barrier
