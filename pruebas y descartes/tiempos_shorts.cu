...
struct less_short {
    __device__ __host__ bool operator()(const short &a, const short &b) const {
        return a < b;
    }
};
...
int main(int argc, char *argv[]) {
    ...
    short* vector;              // vector datos a ordenar
    ...
    printf("        sec      omp      cub      mgpu     fss      hv1      hv2      btb      mks\n");
    for(int k = 0; k < 10; k++) {
        printf("it %d ", k);
        ...
        {
            short* output = (short*) malloc(n_arrays*ELEMS*sizeof(short));
            memcpy(output, vector, n_arrays*ELEMS*sizeof(short)); // copy of the original array
        ...
        #pragma omp parallel
        {
            short* output = (short*) malloc(ELEMS*sizeof(short));
            // gettimeofday(&tv_start, NULL); 
            #pragma omp for
            for(int j = 0; j < n_arrays; j++) {
        ...
        gettimeofday(&tv_start, NULL);
        {
            ...
            output = (short*) malloc(n_arrays*ELEMS*sizeof(short));

            cudaMalloc(&d_input, n_arrays*ELEMS*sizeof(short));
            cudaMalloc(&d_output, n_arrays*ELEMS*sizeof(short));
            ...
            cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_input, d_output, ELEMS*n_arrays, n_arrays, 
                                                    d_offsets, d_offsets+1, 0, sizeof(short)*8);
            ...
            cudaMemcpy(d_input, vector, n_arrays*ELEMS*sizeof(short), cudaMemcpyHostToDevice);
            cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_input, d_output, ELEMS*n_arrays, n_arrays, 
                                                    d_offsets, d_offsets+1, 0, sizeof(short)*8);
            cudaMemcpy(output, d_output, n_arrays*ELEMS*sizeof(short), cudaMemcpyDeviceToHost);
        ...
        gettimeofday(&tv_start, NULL);
        {
            ...
            cudaMemcpy(d_input, vector, n_arrays*ELEMS*sizeof(short), cudaMemcpyHostToDevice);
            mgpu::segmented_sort(d_input, n_arrays*ELEMS, d_offsets, n_arrays, less_short(), context);
            cudaMemcpy(output, d_input, n_arrays*ELEMS*sizeof(short), cudaMemcpyDeviceToHost);
        ...
        gettimeofday(&tv_start, NULL);
        {
            ...
            cudaMalloc(&d_input, n_arrays*ELEMS*sizeof(short));
            cudaMalloc(&d_values, n_arrays*ELEMS*sizeof(short)); 

            cudaMemcpy(d_input, vector, n_arrays*ELEMS*sizeof(short), cudaMemcpyHostToDevice);
            bb_segsort<short, short>(d_input, d_values, n_arrays*ELEMS, d_offsets, n_arrays);
            cudaMemcpy(output , d_input, n_arrays*ELEMS*sizeof(short), cudaMemcpyDeviceToHost);
        ...
        gettimeofday(&tv_start, NULL);
        {
            ...
            output = (short*) malloc(n_arrays*ELEMS*sizeof(short));
            cudaMalloc(&d_input, (n_arrays/2)*ELEMS*sizeof(short));
            cudaMalloc(&d_output, (n_arrays/2)*ELEMS*sizeof(short));
            ...
            cudaMemcpyAsync(d_input, vector, (n_arrays/2)*ELEMS*sizeof(short), cudaMemcpyHostToDevice, stream);
            cub::DeviceSegmentedRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, d_input, d_output, (n_arrays/2)*ELEMS, (n_arrays/2), 
                                                        d_offsets, d_offsets+1, 0, sizeof(short)*8, stream);
            cudaMemcpyAsync(output, d_output, (n_arrays/2)*ELEMS*sizeof(short), cudaMemcpyDeviceToHost, stream);

            memcpy(output+((n_arrays/2)*ELEMS), vector+((n_arrays/2)*ELEMS), (n_arrays/2)*ELEMS*sizeof(short)); // copy of the original array
        ...
        /* EJECUCION HIBRIDA CON MGPU SEGMENTED SORT EN GPU Y OPENMP CON THRUST EN CPU V2*/
        gettimeofday(&tv_start, NULL);
        {
            short* output;
            short* d_input;
            ...
            output = (short*) malloc(n_arrays*ELEMS*sizeof(short));
            cudaMalloc(&d_input, (n_arrays/4)*ELEMS*sizeof(short));
            ...
            cudaMemcpy(d_input, vector, (n_arrays/4)*ELEMS*sizeof(short), cudaMemcpyHostToDevice);
            mgpu::segmented_sort(d_input, (n_arrays/4)*ELEMS, d_offsets, n_arrays/2, less_short(), context);

            memcpy(output+((n_arrays/4)*ELEMS), vector+((n_arrays/2)*ELEMS), (n_arrays/4)*ELEMS*sizeof(short)); // copy of the original array
            #pragma omp parallel 
            {
            #pragma omp for
            ...
            cudaMemcpy(output, d_input, (3*(n_arrays/4))*ELEMS*sizeof(short), cudaMemcpyDeviceToHost);
        ...
        gettimeofday(&tv_start, NULL);
        {
            ...
            for(int i = 0; i < (n_arrays*ELEMS); i++) { // h_vector = [31:16] (id_array) [15:0] (vector) 
	    	    h_vector[i] = (short) ((h_vector[i]) & 65535);
	        }
