...
int main(void) {
  ...
  initialize_vector_f32(h_input, Elems);
  ...
  memcpy(h_output_sec, h_input, Elems*sizeof(float)); // copy of the original array
  qsort(h_output_sec, Elems, sizeof(float), compare_f32);
  ...
  int ret = compare_vector_f32(h_output_sec, h_output_par, Elems); 
  if(ret) {
    ...
    printf("Input:            ");
    for(int i = 0; i < 8; i++) printf(" %f", h_input[i+(Elems/2)]);
    printf(" ...\n"); 
    printf("CPU calculated:   ");
    for(int i = 0; i < 8; i++) printf(" %f", h_output_sec[i+(Elems/2)]);
    printf(" ...\n"); 
    printf("GPU calculated:   ");
    for(int i = 0; i < 8; i++) printf(" %f", h_output_par[i+(Elems/2)]);
    printf(" ...\n");
